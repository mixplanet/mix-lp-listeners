...
interface ITurntableLPListeners {
    ...
    event Claim(uint256 indexed turntableId, address indexed to, address token, uint256 claimed);
    ...
    function accumulativeOf(uint256 turntableId, address owner, address token) external view returns (uint256);
    function claimedOf(uint256 turntableId, address owner, address token) external view returns (uint256);
